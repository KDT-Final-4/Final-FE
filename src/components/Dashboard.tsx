...
export function Dashboard() {
    ...
    const chartData = useMemo(() => {
        // 클릭 추이 차트용 데이터: 생성일 기준 정렬 후 날짜 라벨/클릭수로 매핑
                ...
                <Card>
                  ...
                  {loading ? '불러오는 중...' : error ? '오류' : '실시간'}
                        ...
                        {error ? (
                            ...
                            <p className="text-gray-500 text-sm mt-2">/dashboard/status의 allClicks</p>
                        )}
                    </CardContent>
                </Card>
            </div>
            ...
            <Card className="mb-8">
                ...
                <CardContent>
                    {loading ? (
                        ...
                        <p className="text-red-500">차트 데이터를 불러오지 못했습니다.</p>
                    ) : chartData.length === 0 ? (
                ...
                <table className="w-full text-base">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {recentItems.map((item) => (
                      ...
                      >
                        <td
                          className="px-6 py-4 text-gray-900 font-medium truncate min-w-0"
                          title={item.title || '제목 없음'}
                        >
                          {item.title || '제목 없음'}
                        </td>
                        <td className="px-6 py-4 text-gray-700">{item.createdLabel}</td>
